...
import { useEvaluateInternshipReport } from "@/hooks/useEvaluateInternshipReport";
import { useSupervisorReports } from "@/hooks/SupervisorHook/useSupervisorReports";
...
export default function InternshipMonitoring() {
  ...
  const handleConfirmReview = async () => {
    if (selectedReport) {
      const result = await evaluateReport(selectedReport.id, "hr-evaluate", {
        score: marks,
        feedback: feedback,
      });
      toast.message(
        result.success ? "Evaluation submitted successfully!" : result.error,
        {
  ...
  const { evaluateReport, isLoading } = useEvaluateInternshipReport();
