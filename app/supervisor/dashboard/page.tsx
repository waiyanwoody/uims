  ...
  const stats = dashboardData?.stats ? [
    {
      ...
      value: dashboardData.stats.assignedStudents.toString(),
    ...
    },
    {
      ...
      value: dashboardData.stats.pendingApproval.toString(),
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-card to-secondary/20 p-6 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="space-y-4 animate-fadeIn">
          <div>
            <h1 className="text-4xl font-bold text-foreground">Supervisor Portal</h1>
            <p className="text-muted-foreground mt-1">Monitor student internships and progress</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="p-6 border-border animate-slideInLeft">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-foreground">Student Internships</h2>
                  <p className="text-xs text-muted-foreground mt-1">Current placements and progress</p>
                </div>
              ...
              <div className="space-y-3">
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="p-4 rounded-lg bg-secondary animate-shimmer h-24"></div>
                  ))
                ) : (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs font-semibold opacity-70">{student.startDate}</span>
                        </div>
            ...
            <Card className="p-6 border-border bg-gradient-to-br from-primary/5 via-transparent to-accent/5 animate-slideInLeft">
              ...
              <div className="space-y-3">
                {getDepartmentStats.map((dept, idx) => (
                  <div key={idx} className="p-3 rounded-lg bg-white/50 hover:bg-white/70 transition-colors animate-slideInUp" style={{ animationDelay: `${idx * 50}ms` }}>
                    <div className="flex justify-between items-start mb-2">
                      <p className="font-semibold text-foreground text-sm">{dept.dept}</p>
                      <span className="text-xs font-bold text-primary">{dept.total} students</span>
                    </div>
                    <div className="flex gap-2 text-xs">
                      <div className="flex-1">
                        <p className="text-muted-foreground mb-1">Active</p>
                        <div className="w-full h-2 bg-blue-200 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-500" style={{ width: `${(dept.active / dept.total) * 100}%` }}></div>
                        </div>
                      </div>
                      <div className="flex-1">
                        <p className="text-muted-foreground mb-1">Completed</p>
                        <div className="w-full h-2 bg-emerald-200 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-500" style={{ width: `${(dept.completed / dept.total) * 100}%` }}></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6 border-border bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200 animate-slideInRight">
              ...
              <div className="space-y-3">
                {[
                  { metric: 'Avg Performance', value: '8.5/10', color: 'text-emerald-600' },
                  { metric: 'Completion Rate', value: '87%', color: 'text-blue-600' },
                  { metric: 'Satisfaction', value: '92%', color: 'text-purple-600' }
                ].map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center p-2 bg-white/50 rounded-lg animate-slideInRight" style={{ animationDelay: `${idx * 50}ms` }}>
            ...
            <Card className="p-6 border-border animate-slideInRight">
              ...
              <div className="space-y-2">
                {[
                  { action: 'Review 2 new internships', priority: 'high', icon: AlertCircle },
                  { action: 'Approve 1 application', priority: 'medium', icon: CheckCircle },
                  { action: 'Follow-up with 3 students', priority: 'low', icon: Users }
                ].map((item, idx) => {
                  const Icon = item.icon
                  const colors = item.priority === 'high' 
                  ...
                  return (
                    <div key={idx} className={`p-3 rounded-lg border ${colors} hover:shadow-md transition-all animate-slideInUp`} style={{ animationDelay: `${idx * 50}ms` }}>
