...
export default function LoginPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<'student' | 'hr' | 'supervisor'>('student')

  // Mock user credentials
  const mockCredentials = {
    student: { email: 'john.student@university.edu', password: 'password123' },
    hr: { email: 'hr@techcorp.com', password: 'password123' },
    supervisor: { email: 'prof.supervisor@university.edu', password: 'password123' }
  }
      ...
      <div className="w-full max-w-5xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="hidden lg:flex flex-col justify-center space-y-8">
            <div>
              <h2 className="text-4xl font-bold text-foreground mb-4">UIMS</h2>
            ...
            <div className="space-y-6">
              {demoAccounts.map((account, index) => (
                <div
                  key={index}
                  className="p-4 rounded-lg border border-border bg-card/50 hover:bg-card hover:border-primary/30 cursor-pointer transition-all"
                  onClick={() => fillDemoCredentials(account.role)}
                >
                  <div className="font-semibold text-foreground">{account.name}</div>
                  <div className="text-sm text-muted-foreground">{account.position}</div>
                  <div className="text-xs text-primary mt-2">{account.email}</div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-border space-y-3">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    Supervisor: prof.supervisor@university.edu / password123
                  </p>
                </div>
              </div>
